...
function MatrixCell({ status }: { status: CompStatus }) {
  ...
  const mod =
    ...
    status === 'RPL-G' ? 'rpl' :
    status === 'CT'    ? 'ct' : 'nys'
  return <span className={`wsr-matrix-cell wsr-matrix-cell--${mod}`}>{status}</span>
}
...
function ProgressCell({ pct }: { pct: number }) {
  const mod =
    ...
    pct >= 40   ? 'mid'  :
    pct > 0     ? 'low'  : 'zero'
  return <span className={`wsr-matrix-cell wsr-matrix-cell--pct-${mod}`}>{pct}%</span>
}

function EnrolCell({ status }: { status: 'A' }) {
  return <span className="wsr-matrix-cell wsr-matrix-cell--c">{status}</span>
}
...
export function WorkshopRefreshPage() {
  ...
  function toggleAll() {
    setSelectedIds(allSelected ? new Set() : new Set(LEARNERS.map(l => l.id)))
  }
        ...
        <div className="wsp-main">
          ...
          <div className="wsp-table-wrap">
            <table className="wsr-table">
              <thead>
                <tr>
                  <th className="wsr-th wsr-th--check">
                    <Checkbox checked={allSelected} onCheckedChange={toggleAll} />
                  </th>
                  <th className="wsr-th wsr-th--learner">Learners</th>
                  <th className="wsr-th wsr-th--center">Enrolment<br />status</th>
                  <th className="wsr-th wsr-th--center">Unit<br />progress</th>
                  {UNIT_COLUMNS.map(col => (
                    <th key={col.code} className="wsr-th wsr-th--unit">
              ...
              <tbody>
                {visibleLearners.map(learner => (
                  <tr
                    key={learner.id}
                    className={`wsr-row${selectedIds.has(learner.id) ? ' wsr-row--selected' : ''}`}
                  >
                    <td className="wsr-td wsr-td--check">
                      <Checkbox
                        checked={selectedIds.has(learner.id)}
                        onCheckedChange={() => toggleRow(learner.id)}
                      />
                    </td>
                    <td className="wsr-td wsr-td--learner">
                      <div className="wsp-learner-cell">
                        <Avatar mode="image" shape="circle" theme="flat" src={learner.avatarImg} alt={learner.name} />
                    ...
                    <td className="wsr-td wsr-td--cell">
                      <EnrolCell status={learner.enrolStatus} />
                    </td>
                    <td className="wsr-td wsr-td--cell">
                      <ProgressCell pct={learner.unitProgress} />
                    </td>
                    {learner.units.map((status, i) => (
                      <td key={i} className="wsr-td wsr-td--cell">
